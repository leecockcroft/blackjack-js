    const app = (() => {
      ...
      const deck = () => {
        ...
        for (var i = 0; i < cache.fullDeck.length; i++) {
          ...
          if (cache.fullDeck[i].value == 'A') {cache.fullDeck[i].weight = 11}
      ...
      const deal = () => {
        ...
        setTimeout(() => {
          if (cache.dealerScore == 21) {alert('blackjack')}
          if (cache.playerScore == 21) {alert(' player has blackjack')}
        },2000)
    }
      ...
      const hit = () => {
        ...
        let newCard;
        for (var i = 0; i < remainingPack.length; i++) {newCard = remainingPack[i]}
        ...
        while (cache.dealerScore < 17 & cache.dealerScore < 21) {
          var cardClick = (function() {
            return function cardClicks(e) {
              ...
              let nextCardDisplay;
              let remainingPack = document.querySelectorAll('.stacked_card')
              for (var i = 0; i < remainingPack.length; i++) {nextCardDisplay = remainingPack[i]}
